...
-----------------------------------------------------------------------------------------------------------
--העברת כל הרכבים המבצעיים משנת ייצור קטנה מ-2015 לתחנה בנגב (נניח מזהה תחנה 3)
UPDATE Vehicle
SET Station_ID = 3
WHERE Manufacture_Year < 2015 
...
WHERE Supplier_ID = 101 AND Delivery_Date >= CURRENT_DATE;--עדכון כמות המלאי (תוספת 10%) לפריטים רפואיים בתחנות ירושלים
UPDATE Station_Inventory
SET Quantity = Quantity * 1.10
WHERE Station_ID IN (SELECT Station_ID FROM Station WHERE City = 'Jerusalem')
AND Item_Code IN (SELECT Item_Code FROM Equipment_Catalog WHERE Category = 'Medical Device');
